...
public class Grid implements Serializable {
        ...
        public boolean isPossible(Cell cell) {
                ...
                // linhas - Row
                for (int r = 0; r < getSize(); r++) {
                        if (value == grid[r][col].getValue() && r != row && !grid[r][col].isBadValue()) {
                                return false;
                        }
                }
                // colunas - cols
                for (int c = 0; c < getSize(); c++) {
                        if (value == grid[row][c].getValue() && c != col && !grid[row][c].isBadValue()) {
                                return false;
                        }
                }
                //Quadrado
                ...
                for (int i = start_row * 3; i < (start_row * 3) + 3; i++) {
                        for (int j = start_col * 3; j < (start_col * 3) + 3; j++) {
                                if (value == grid[i][j].getValue() && (i != row && j != col) && !grid[i][j].isBadValue()) {
                                        return false;
                                }
                        }
                }
        ...
        public void clearEarnedPoints() {
                for (int row = 0; row < getSize(); row++) {
                        for (int col = 0; col <getSize() ; col++) {
                                grid[row][col].clearEarnedPoints();
                        }
                }
        }
}
